...
#define GPIO_PORT_DATA        (*(volatile uint16_t *)0x80000C4)
#define GPIO_PORT_DIRECTION   (*(volatile uint16_t *)0x80000C6)
...
void gba_rumble_gpio_update(bool rumble_on) {
    if (rumble_on) {
        GPIO_PORT_DATA |= (1 << 3);
    } else {
        GPIO_PORT_DATA &= ~(1 << 3);
...
void gba_rumble_ezode_update(bool rumble_on) {
    // This also seems to control the ChisFlash V1.2 Rumble, in addition to the more standard GPIO control.
